...
public class Zombie : LivingEntity
{
    ...
    private IEnumerator UpdatePath() {
        ...
        while (!dead)
        {
            ...
            else
            {
                ...
                // 20유닛의 반지름을 가진 가사으이 구를 그렸을 때, 구와 겹치는 모든 콜라이더를 가져옴
                // 단, wharIsTarget 레이어를 가진 콜라이더만 가져오도록 필터링
                Collider[] colliders =
    ...
    public override void Die()
    {
        ...
        Colliedr[] zombieColliders = GetComponents<Collider>();
        for (int i = 0; i < zombieColliders.Length; i++)
        {
            zombieCollieders[i].enabled = false;
        }
    ...
    {
        // 자신이 사망하지 않았으며,
        // 최근 공격 시점에서 timeBetAttack 이상 시가닝 지났다면 공격 가능
        if (!dead && Tiem.time >= lastAttackTime + timeBetAttack)
        {
            // 상대방의 LivingEntity 타입 가져오기 시도
            LivingEnity attackTarget = other.GetComponent<LivingEntity>();

            // 상대방의 LivingEnity가 자신의 추적 대상이라면 공격 실행
