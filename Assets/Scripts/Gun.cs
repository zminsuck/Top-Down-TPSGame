...
public class Gun : MonoBehaviour {
    ...
    [Header("Mode B")]
    public AudioClip shotClipB; // 모드 B 발사소리
    ...
    private void OnEnable()
    {
        // 총 상태 초기화
        // 전체 예비 탄알 양을 초기화
        ammoRemain = gunData.stateAmmoRemain;
        //현재 탄창을 가득 채우기
    ...
    private void Shot()
    {
        // 래이캐스트에 의한 충돌 정보를 저장하는 컨테이너
        RaycastHit hit;
        // 탄알이 맞은 곳을 저장할 변수
        Vector3 hitPosition = Vector3.zero;

        // 래이캐스트 (시작 지점, 방향, 충돌 정보 컨테이너, 사정거리)
        if (Physics.Raycast(fireTransform.position, fireTransform.forward, out hit, fireDistance))
        {
            // 레이가 어떤 물체와 충돌한 경우
            // 충돌한 상대방으로부터 IDamageable 오브젝트를 가져오기 시도
            IDamageble target = hit.collider.GetComponent<IDamageable>();
        ...
        // 발사 이펙트 재생 시작
        StateCorouitine(ShotEffect(hitPosition));
